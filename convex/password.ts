  ...
  export function Password<DataModel extends GenericDataModel>(
    ...
    return ConvexCredentials<DataModel>({
      ...
      authorize: async (params, ctx) => {
        ...
        if (flow === "signUp") {
          if (secret === undefined) {
            throw new ConvexError("Missing `password` param for `signUp` flow");
          }
          ...
          if (secret === undefined) {
            throw new ConvexError("Missing `password` param for `signIn` flow");
          }
          ...
          if (!config.reset) {
            throw new ConvexError(`Password reset is not enabled for ${provider}`);
          }
          ...
          if (!config.verify) {
            throw new ConvexError(`Email verification is not enabled for ${provider}`);
          }
          const { account } = await retrieveAccount(ctx, {
            provider,
            account: { id: email },
