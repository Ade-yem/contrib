import {
  ...
  >(
    ...
    return await scheduleCron(ctx, { kind: "interval", ms }, func, limit, name, ...args);
  }
  ...
  /**
   * checks if a function should be scheduled
   * @param count count of the job
   * @param limit limit of the job
   * @returns true if the count is less than the limit or false
   */
  function checkTobeScheduled(count: number, limit: number): boolean {
