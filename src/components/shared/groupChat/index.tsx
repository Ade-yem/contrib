...
export const GroupChat = ({groupId, userId}: {groupId: Id<"groups">; userId: Id<"users">}) => {
  const {results, status, loadMore} = usePaginatedQuery(api.chat.getChatsInGroup, {groupId}, {initialNumItems: 100});
  ...
  const sendMessage = async () => {
    if (image) {
      ...
      await sendChat({
        groupId, userId, message, imageId: storageId as Id<"_storage">
      })
    } else await sendChat({
    ...
    <div className="chat-support-wrapper">
      ...
      <div className="comment-footer">
        <textarea
          ...
          value={message}
        ></textarea>
        <input type="file" name="chatFile" id="chatFile" hidden ref={inputRef} onChange={(e) => {
