...
export default function GroupDetails({params}: {params: {code: string}}) {
  ...
  const group = useQuery(api.group.getGroupWithInviteCode, {code: params.code})
  const creator = useQuery(api.user.getUserzById, {userId: group!?.creator_id})
  ...
  return(
    <div className="col-lg-3 col-md-4 col-sm-6 col-12 my-5_6">
      <div
        className={`default-card outstanding-group-card`}
      >
        <div
        ...
        <div className="d-flex justify-content-between align-items-center px-5 mb-4 padlock-price">
          ...
          <span className="price d-non">
            <span>₦{thousandFormatter(group?.savings_per_interval ?? 0 / 100)}</span>
          </span>
        </div>
      ...
      <br />
      <button onClick={joinGroup} className="btn btn-md btn-primary  mx-auto">Join Group</button>
    </div>
  );
}
